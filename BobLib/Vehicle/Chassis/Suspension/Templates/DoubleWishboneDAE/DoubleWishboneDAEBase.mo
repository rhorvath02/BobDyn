within BobLib.Vehicle.Chassis.Suspension.Templates.DoubleWishboneDAE;

partial model DoubleWishboneDAEBase
  // Modelica units
  ...
  Modelica.Mechanics.MultiBody.Interfaces.Frame_b midpoint_frame annotation(
    Placement(transformation(origin = {-100, 0}, extent = {{16, -16}, {-16, 16}}), iconTransformation(origin = {-100, 0}, extent = {{-16, -16}, {16, 16}})));
  // Upper wishbone
  final Modelica.Mechanics.MultiBody.Joints.Revolute upper_inboard_joint(animation = false, n = normalize(upper_fore_i - upper_aft_i), phi(displayUnit = "rad")) annotation(
    Placement(transformation(origin = {60, 60}, extent = {{10, -10}, {-10, 10}})));
  final Modelica.Mechanics.MultiBody.Parts.FixedTranslation upper_rigid_link(animation = false, shapeType = "cylinder", r = upper_o - (upper_fore_i + upper_aft_i)/2, extra = 0.0) annotation(
    Placement(transformation(origin = {30, 60}, extent = {{10, -10}, {-10, 10}})));
  // Lower wisbone
  final Modelica.Mechanics.MultiBody.Joints.Revolute lower_inboard_joint(animation = false, n = normalize(lower_fore_i - lower_aft_i), phi(displayUnit = "rad")) annotation(
    Placement(transformation(origin = {60, -60}, extent = {{10, -10}, {-10, 10}})));
  final Modelica.Mechanics.MultiBody.Parts.FixedTranslation lower_rigid_link(animation = false, r = lower_o - (lower_fore_i + lower_aft_i)/2, shapeType = "cylinder", extra = 0.0) annotation(
    Placement(transformation(origin = {30, -60}, extent = {{10, -10}, {-10, 10}})));
  // Upright
  final BobLib.Vehicle.Chassis.Suspension.Linkages.Upright upright(lower = lower_o, upper = upper_o, tie = tie_o) annotation(
  ...
  // UCA mass + inertia
  final Modelica.Mechanics.MultiBody.Parts.Body UCA_body(animation = true, r_CM = uca_mass.r_cm - upper_o, m = uca_mass.m, I_11 = uca_mass.I[1, 1], I_22 = uca_mass.I[2, 2], I_33 = uca_mass.I[3, 3], I_21 = uca_mass.I[2, 1], I_31 = uca_mass.I[3, 1], I_32 = uca_mass.I[3, 2], sphereDiameter = joint_diameter, cylinderDiameter = link_diameter, angles_start = {0, 0, 0}, w_0_start = {0, 0, 0}, z_0_start = {0, 0, 0}, useQuaternions = false) annotation(
    Placement(transformation(origin = {-10, 60}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  // LCA mass + inertia
  final Modelica.Mechanics.MultiBody.Parts.Body LCA_body(r_CM = lca_mass.r_cm - lower_o, m = lca_mass.m, I_11 = lca_mass.I[1, 1], I_22 = lca_mass.I[2, 2], I_33 = lca_mass.I[3, 3], I_21 = lca_mass.I[2, 1], I_31 = lca_mass.I[3, 1], I_32 = lca_mass.I[3, 2], sphereDiameter = joint_diameter, cylinderDiameter = link_diameter, angles_start = {0, 0, 0}, w_0_start = {0, 0, 0}, z_0_start = {0, 0, 0}, useQuaternions = false) annotation(
    Placement(transformation(origin = {-10, -60}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  // Spherical joints
  Modelica.Mechanics.MultiBody.Joints.Spherical upper_spherical(sphereDiameter = joint_diameter, useQuaternions = false) annotation(
    Placement(transformation(origin = {10, 30}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  //                                                 trans_x_stiffness = 1e7,
  //                                                 trans_y_stiffness = 1e7,
  //                                                 trans_z_stiffness = 1e7,
  //                                                 trans_x_damping = 3e3,
  //                                                 trans_y_damping = 3e3,
  //                                                 trans_z_damping = 3e3,
  //                                                 diameter = joint_diameter,
  //                                                 r_rel(start = {0, 0, 0}, fixed = true)
  Modelica.Mechanics.MultiBody.Joints.Spherical lower_spherical(sphereDiameter = joint_diameter) annotation(
    Placement(transformation(origin = {10, -30}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
  //                                               trans_x_stiffness = 1e8,
  ...
  //                                               trans_z_damping = 1e4,
  //                                               diameter = joint_diameter,
  //                                               r_rel(start = {0, 0, 0}, each fixed = true)
  // Steering interface
