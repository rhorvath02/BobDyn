...
model MF5p2Tire
  ...
  // Custom linalg
  import BobDyn.Utilities.Math.Vector.angle_between;
  import BobDyn.Utilities.Math.Vector.cross;
  import BobDyn.Utilities.Math.Vector.dot;
  // MF52 functions
  import BobDyn.Vehicle.Chassis.Tires.MF52.Records.Mz_record;
  import BobDyn.Vehicle.Chassis.Tires.MF52.Fx_eval;
  import BobDyn.Vehicle.Chassis.Tires.MF52.Fy_eval;
  import BobDyn.Vehicle.Chassis.Tires.MF52.Mx_eval;
  import BobDyn.Vehicle.Chassis.Tires.MF52.My_eval;
  import BobDyn.Vehicle.Chassis.Tires.MF52.Mz_eval;
  // Parameters - Tire defn
  final parameter BobDyn.Resources.Records.TIRES.Fr_tire tire "MF52 tire parameter record" annotation();
  ...
  parameter Real FNOMIN = tire.FNOMIN "Nominal normal load, FZ0";
...
protected
  // Numerical stability
  parameter Real v_min = 0.1 "Low-speed threshold for force gating (m/s)" annotation(
    Dialog(group = "Numerical Conditions"));
  parameter Real eps = 1e-6 "Small constant to prevent division by zero" annotation(
    Dialog(group = "Numerical Conditions"));
  // Diagnostics
  Real P_contact;
